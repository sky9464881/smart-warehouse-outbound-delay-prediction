...
async function handleSubmit() {
  ...
  try {
    ...
    localError.value = auth.errorMessage || error.message || 'Login failed.'
  } finally {
    ...
    <section class="auth-card" aria-label="Login">
      ...
      <form class="auth-form" @submit.prevent="handleSubmit">
        ...
        <button class="submit" type="submit" :disabled="auth.isLoading || isSubmitting">
          {{ auth.isLoading || isSubmitting ? '로그인 중...' : '로그인' }}
        </button>
      </form>

      <footer class="auth-foot">
        <span>계정이 없나요?</span>
        <RouterLink class="link" :to="{ name: 'signup', query: { redirect: redirectTo } }">회원가입</RouterLink>
      </footer>
    </section>
  </main>
</template>
